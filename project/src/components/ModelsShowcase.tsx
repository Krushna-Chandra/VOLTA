...
export const ModelsShowcase = ({ t }: ModelsShowcaseProps) => {
  ...
  const nextModel = () => {
    setCurrentModelIndex((prev) => (prev + 1) % vehicleModels.length);
  };

  const prevModel = () => {
    setCurrentModelIndex((prev) => (prev - 1 + vehicleModels.length) % vehicleModels.length);
  };
  ...
  const handleReserve = () => {
    alert(`Configuration saved!\nModel: ${currentModel.name}\nColor: ${colorOptions.find(c => c.id === config.color)?.name}\nWheels: ${wheelOptions.find(w => w.id === config.wheels)?.name}\nInterior: ${interiorOptions.find(i => i.id === config.interior)?.name}`);
  };
    ...
    >
      <div className="container mx-auto px-4">
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className={`relative overflow-hidden rounded-3xl shadow-2xl transition-all duration-1000 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            <div className="relative h-[600px]">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
                <div className="max-w-2xl">
                  <h3 className="text-5xl font-bold text-white mb-2">{currentModel.name}</h3>
                  <p className="text-2xl text-slate-300 mb-8">{currentModel.tagline}</p>

                  <div className="grid grid-cols-3 gap-6 mb-8">
                    <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
                      <Zap className="h-6 w-6 text-blue-400 mb-2" />
                      <div className="text-3xl font-bold text-white">{currentModel.range} km</div>
                      <div className="text-sm text-slate-300">{t('specs.range')}</div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
                      <Gauge className="h-6 w-6 text-blue-400 mb-2" />
                      <div className="text-3xl font-bold text-white">{currentModel.acceleration}s</div>
                      <div className="text-sm text-slate-300">{t('specs.acceleration')}</div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
                      <Clock className="h-6 w-6 text-blue-400 mb-2" />
                      <div className="text-3xl font-bold text-white">{currentModel.chargeTime} min</div>
                      <div className="text-sm text-slate-300">{t('specs.chargeTime')}</div>
                    </div>
                  </div>
            ...
            <button
              onClick={prevModel}
              className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 backdrop-blur-md hover:bg-white/20 rounded-full transition-all border border-white/20"
            ...
            <button
              onClick={nextModel}
              className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 backdrop-blur-md hover:bg-white/20 rounded-full transition-all border border-white/20"
        ...
        {showConfigurator && (
          <div className="mt-12 max-w-4xl mx-auto bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 animate-fade-in">
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-wrap gap-4">
                  {colorOptions.map((color) => (
                    <button
                      ...
                      className={`group relative flex flex-col items-center space-y-2 p-4 rounded-xl transition-all ${
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {wheelOptions.map((wheel) => (
                    ...
                    >
                      <div className="text-4xl mb-2">{wheel.image}</div>
                      <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                        {wheel.name}
                      </span>
                    </button>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4">
                  {interiorOptions.map((interior) => (
                    ...
                    >
                      <div
                        className="h-12 w-12 rounded-lg border-4 border-white shadow-lg"
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-slate-700">
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-full font-semibold transition-all transform hover:scale-105 shadow-lg"
                >
